 ...
 *
 * Admin-only. Evaluates all alert conditions against the AnalyticsEvent table
 * and returns their current firing status.
 *
 * Response:
 *   { alerts: AlertResult[], summary: { firing, ok, insufficientData } }
 */

type AlertSeverity = 'P1' | 'P2' | 'P3';
type AlertStatus = 'ok' | 'firing' | 'insufficient_data';
...
export async function GET() {
  ...
  try {
    ...
    // In last 2h: workflow_uploaded / (workflow_uploaded + upload_failed) < 0.95
    ...
    // In last 7 days: users with sop_section_viewed / users with signup_completed < 0.20
    const cutoff7d = daysAgo(7);
    // Count distinct users who signed up and who viewed a sop section
    // Using groupBy on userId to count distinct active users
    ...
    // In last 7 days: sop_usefulness_response events where properties.response is
    // "yes_as_is" or "minor_edits" should be >= 40% of total (only if total > 5)
