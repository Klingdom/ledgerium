  ...
  if (!resolved && !isLoading) {
    return (
      <div className="text-center py-ds-12">
        ...
        <h3 className="mt-ds-4 text-ds-base font-medium text-[var(--content-primary)]">Run process intelligence</h3>
    ...
    <div className="ds-document">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-ds-4">
        <div className="card px-ds-4 py-ds-3">
          ...
          <p className="text-ds-xs text-[var(--content-tertiary)]">median</p>
        </div>
        <div className="card px-ds-4 py-ds-3">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-ds-sm font-medium text-[var(--content-primary)]">
                      Step {b.position} <span className="text-ds-xs text-[var(--content-secondary)]">({b.category})</span>
                    </p>
                    <p className="text-ds-xs text-[var(--content-secondary)] mt-0.5">
                  ...
                  <div className="text-right">
                    ...
                    <p className="text-ds-xs text-[var(--content-tertiary)]">vs {formatDuration(b.overallMeanStepDurationMs)} avg</p>
                  </div>
                </div>
              </div>
            ))}
              ...
              <div key={v.variantId} className={`card px-ds-5 py-ds-3 ${v.isStandardPath ? 'border-brand-200 bg-brand-50/30' : ''}`}>
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <p className="text-ds-xs text-[var(--content-tertiary)] mt-0.5 font-mono truncate max-w-sm">
                      {v.pathSignature.signature}
                    </p>
                  </div>
                  <div className="text-right">
                    <p className="text-ds-sm font-semibold text-[var(--content-primary)]">{Math.round(v.frequency * 100)}%</p>
                    <p className="text-ds-xs text-[var(--content-tertiary)]">{v.runCount} run{v.runCount !== 1 ? 's' : ''}</p>
                  </div>
                </div>
              </div>
            ))}
