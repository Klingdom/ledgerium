...
export function InsightActionCard({
  ...
  const border = SEVERITY_BORDER[insight.severity] ?? 'border-l-gray-300';
  const badge = SEVERITY_BADGE[insight.severity] ?? 'bg-gray-50 text-gray-600 border border-gray-200';
  ...
  return (
    <div
      className={`border-l-[3px] ${border} bg-white border border-gray-100 rounded-ds-md overflow-hidden`}
    >
      {/* Header row — always visible */}
      <button
        type="button"
        onClick={() => hasDetails && setExpanded((v) => !v)}
        className={`w-full text-left px-4 py-3 flex items-start gap-3 ${hasDetails ? 'cursor-pointer hover:bg-gray-50/60 transition-colors' : 'cursor-default'}`}
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-ds-sm font-medium text-gray-900 leading-snug">{insight.title}</p>
          <p className="text-ds-xs text-gray-500 mt-0.5 line-clamp-2">{insight.description}</p>
            ...
            <div className="mt-1.5 flex items-center gap-1 flex-wrap">
              ...
              {insight.stepOrdinals.map((ordinal) => (
                <button
                  ...
                  className={`inline-flex items-center justify-center h-5 w-5 rounded-full text-[10px] font-semibold bg-gray-100 text-gray-600 ${onStepClick ? 'hover:bg-brand-100 hover:text-brand-700 cursor-pointer' : 'cursor-default'}`}
                >
                  {ordinal}
                </button>
              ))}
          ...
          <div className="px-4 pb-4 space-y-3 border-t border-gray-100 pt-3">
            {insight.evidence && (
              <div>
                ...
                <p className="text-ds-xs text-gray-700">{insight.evidence}</p>
              </div>
            )}
            {insight.impact && (
              <div>
                ...
                <p className="text-ds-xs text-gray-700">{insight.impact}</p>
              </div>
            )}
            {insight.suggestion && (
              <div className="bg-blue-50 rounded-ds-md px-3 py-2.5">
                ...
                <p className="text-ds-xs text-gray-700">{insight.suggestion}</p>
              </div>
            )}
          </div>
        </div>
