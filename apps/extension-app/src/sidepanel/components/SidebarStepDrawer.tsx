...
export function SidebarStepDrawer({ stepDef, sopStep, onClose }: SidebarStepDrawerProps) {
  return (
    <div className="flex flex-col h-full bg-white border-l border-gray-200">
      {/* Header */}
      <div className="flex items-start gap-2 px-3 py-3 border-b border-gray-200 flex-none">
        <div
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-semibold text-gray-900 leading-snug">{stepDef.title}</p>
          <span
            className="inline-block mt-1 text-xs font-bold uppercase tracking-wider rounded px-1.5 py-0.5"
            style={{
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        ...
        <Section title="Operational Definition">
          <p className="text-xs text-gray-600 leading-relaxed bg-gray-50 rounded-lg p-2.5 border border-gray-200">
          ...
          <Section title="Procedure">
            <div className="bg-gray-50 rounded-lg border border-gray-200 overflow-hidden">
              {sopStep.detail.split('\n').filter(Boolean).map((line, i) => (
                <div key={i} className="flex gap-2 px-2.5 py-2 border-b border-gray-100 last:border-0">
                  <span className="text-xs font-bold text-gray-400 tabular-nums mt-0.5 shrink-0 w-3">{i + 1}</span>
        ...
        {sopStep && (
          <Section title="Expected Outcome">
            <p className="text-xs text-gray-600">{sopStep.expectedOutcome}</p>
          </Section>
        )}
          ...
          <Section title="Systems Used">
            <div className="flex flex-wrap gap-1.5">
              {stepDef.systems.map(sys => (
                <span
                  key={sys}
                  className="text-xs text-gray-600 bg-gray-100 border border-gray-200 rounded px-2 py-0.5"
                >
                  {sys}
                </span>
              ))}
        ...
        {sopStep && sopStep.warnings.length > 0 && (
          <Section title="Privacy Notes">
            {sopStep.warnings.map((w, i) => (
              <p key={i} className="text-xs text-amber-600 bg-amber-50 border border-amber-200 rounded px-2 py-1.5 leading-snug">
                {w}
              </p>
            ))}
          </Section>
