...
Use the full selection policy from `CLAUDE.md § Selection Policy`:
...
Range after refinement: roughly 6–18 (was 10–16). Applied since Meta-Review 001 (iter 009+).

---

## Step 5 — Select ONE Item

Portfolio rules (any OVERRIDES top-score):
...
**Agent diversity rule:** if 3 consecutive iterations used the same implementing agent, the 4th MUST use a different one OR the coordinator must explicitly justify the repetition in `ITERATION_LOG.md` under "Agents Used."

### Strict Rules

- Only implement selected item
- No scope expansion
- Record chosen delegate + rationale in iteration log
...
See `CLAUDE.md § Meta-Review Cadence` for the authoritative rule set.

Call `meta-coordinator` when:

**Base cadence:**
- Every 3 completed improvement loops (Mode 5 directed sequences increment by N items, not by 1 batch)

**Early triggers** (any one forces an immediate meta-review):
...
After a meta-review, do not run another for at least 3 loops — changing multiple control variables more often than that makes effectiveness measurement impossible.
