...
**I1. LiveStep-to-DerivedStep correspondence.** For every fixture, running events through the live adapter and running the same events through the batch adapter must produce `DerivedStep` lists (extracted from each path) that match for their finalized entries. This is a stronger form of invariant 3.7 and is the behavioural guarantee the convergence delivers. Assert: `liveFinalizedDerivedSteps === batchDerivedSteps` for the same canonical event sequence.
