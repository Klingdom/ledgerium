...
- **P0-2 (product/UX) — Extension activation requires manual Chrome Developer-Mode sideload** (download zip → extract → enable dev mode → load unpacked), not a Web Store install. Serious self-serve drop-off at peak post-signup intent; contradicts CLAUDE.md's Web Store submission goal.
- **P0-3 (analytics) — Signup→install funnel is completely dark.** The `/install` "Download Chrome Extension" button is a bare `<a href download>` with **zero click tracking** and no downstream extension-load signal — cannot distinguish "never clicked" vs "downloaded, never loaded" vs "loaded, never recorded." The single most important activation step is unmeasurable.
...
- **P1-2 (ux) — Sitewide dead footer link.** `Footer.tsx:10` "Interactive Demo" → `/dashboard.html` (no matching route) — a 404 on every page's footer.
