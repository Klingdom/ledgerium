...
- Follow-up pool at **28 items** post-iter-032 (unchanged from iter-031 close; Mode 4 MR-007 is zero code; cold-pool promotions re-anchor existing live rows #34/#35/#36 — not new additions). Pool-size ceiling rule still violated (pool > 8 soft; > 15 hard applies in Mode 5 only). MR-005 Change D-2 hard-stop ceiling (pool > 15) applies inside Mode 5 only; no Mode 5 expected until Path C Build Phase A (earliest iter 033+, blocked on PRD approval + pre-check). **Cool-off recharge counter 2/3 UNCHANGED post-iter-032** (Mode 4 non-counting; iter 033 burn-down completes re-arm to 3/3; earliest re-consumption iter 034). Burn-down trajectory per MR-007 § 6: iter 033 #24 (non-web-app burn-down per saturation rule, −1) → 27. Scenario A all-burn-down 033-037 → ~22 by iter 038; Scenario C with MR-007-promoted bundles → ~18 by iter 038. **MR-007 proposes ≤15 by iter 040** (slip MR-006 target 2 iter); **CEO confirmation requested** on Q4 burn-rate target revision.
- **Area saturation STILL TRIPPED at iter 033 entry** — iter 029/030/031 all web-app; Mode 4 iter 032 Area = governance, non-counting for Area cadence. **Iter 033 MUST select from non-web-app Area** per CLAUDE.md Selection Policy Step 2. This is a hard constraint and overrides burn-down top-score-within-burn-down-pool ranking. **MR-007 § 5 endorsed pick #24 LiveStep type tightening (segmentation-engine)** satisfies saturation and clears D-1 counter.
- **DASHBOARD_V2_REVIEW_001 cold pool: 24 items** (11 P1 DV2-R04→R14, 10 P2 DV2-R15→R24, 3 P3 DV2-R25→R27) held in the review artifact per MR-005 D-5 clauses 4+5. Promote via P0 burn-down slot creation OR PRD-trigger enumerated dependency. Most impactful P1s: DV2-R04 axe-core regression gate, DV2-R05 `seedDashboardV2Dev()` + free-tier test user (unblocks 8 skipped E2E tests), DV2-R06 route v1 shadow-function audit (real #42 retirement blocker). Cold-pool age at iter 032: **5 iter**; MR-006 Change D staleness threshold at 10 iter → **reached iter ~036; triaged at MR-008**. **PRICING_AUDIT_001 cold-pool triage COMPLETED at MR-007** — 3 × `promote` rows #34/#35/#36 re-anchored `Birth iter: MR-007-promoted`.
