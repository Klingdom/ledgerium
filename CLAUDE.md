...
- **Iter 028 (NEXT, burn-down, bundle):** #19 + #20 bundled — both extension-app surface, both iter-010 follow-ups past cap, same `storage.ts` SW-startup code path (guardrail 7(b) one-logical-outcome preserved); primary agent rotation candidate if `backend-engineer` streak would reach 4 (currently = 2 post-iter-027; iter 028 backend-engineer = 3; mandatory rotate at iter 029)
...
- Follow-up pool at **34 items** post-iter-027 (35 at intake close − 1 from #7 closure). Pool-size ceiling rule still violated (pool > 8 soft; > 15 hard). MR-005 Change D-2 hard-stop ceiling (pool > 15) applies inside Mode 5 only; no Mode 5 expected through iter 028. Burn-down programming projects pool ≤31 by iter 029 close, ≤28 by iter 031 close. Realistic ≤15 target: iter 038.
...
- Reverse portfolio-drift trigger (MR-005 Change D-1, N=5) **FULLY CLEARED at iter 027 close** — policy-engine regex widening is a D-1-enumerated tracked extension surface touch; 5-consecutive-non-extension counter reset to 0. Next counter check at iter 033 (if iter 028-032 all miss the tracked extension-surface list).
...
- Agent-diversity: `backend-engineer` consecutive counter = **2 post-iter-027**. If iter 028 is also `backend-engineer` (likely, #19+#20 storage.ts burn-down is a backend-engineer surface), counter reaches 3. Iter 029 then becomes a MANDATORY rotation slot (same-implementer-4+ trigger fires if iter 029 is a 4th `backend-engineer`). DV2-R01 (iter 029 recommended) is analytics-ops / server-side script — appropriate for `backend-engineer` OR `analytics` agent; **rotate to `analytics` at iter 029** to satisfy the diversity rule while preserving DASHBOARD_V2_REVIEW_001 programming.
