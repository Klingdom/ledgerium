...
- **Iter 027 (NEXT, MANDATORY burn-down, extension surface):** #7 Widen policy-engine `credit_card` regex — **fully resolves reverse portfolio-drift trigger D-1** (pure policy-engine surface, D-1-enumerated tracked extension surface; E=1/R=1); primary agent `backend-engineer`
- **Iter 028 (burn-down, bundle):** #19 + #20 bundled — both extension-app surface, both iter-010 follow-ups past cap, same `storage.ts` SW-startup code path (guardrail 7(b) one-logical-outcome preserved); primary agent rotation candidate if `backend-engineer` streak would reach 4
- **Iter 029 first `top-score` slot eligible** (cool-off re-arms after 3 consecutive burn-downs 026 ✅ → 027 → 028): **tie-break = DV2-R01 (audit-intake P0, score 13, analytics-ops)** over #51 (score 13) and #4 (score 13) per DASHBOARD_V2_REVIEW_001 § Recommended Iter Sequencing — rationale: DV2-R01 executes without PostHog gating, directly unblocks #42, ~1-day server-side script, produces `docs/analysis/HEALTH_SCORE_DISTRIBUTION_COMPARISON.md` artifact
- **Iter 030:** #51 v2 analytics instrumentation (PRD §4 measurable-outcome dependency, score 13) — 6-event spec per DASHBOARD_V2_REVIEW_001 analytics lens
...
- Post-burn-down pool (iter 032+): #34/35/36 audit-intake P0s, #42 v1 health-score retirement (now unblocked by DV2-R01 closure), #57 v2 flag full retirement (14d-soak + DV2-R02/R03 + #51 prerequisite), DASHBOARD_V2_REVIEW_001 P1 cold-pool promotions (DV2-R04 axe ratchet / DV2-R05 seed fixture + free-tier user / DV2-R06 v1 shadow-function audit)

Follow-up pool is at **35 open items** at DASHBOARD_V2_REVIEW_001 intake close (+3 from P0 promotion; 32 + 3 = 35). Ceiling rule still violated (35 > 8 soft; 35 > 15 hard). MR-005 Change D-2 hard-stop ceiling (pool > 15) applies inside Mode 5 only; no Mode 5 expected through iter 028. Post-intake burn-down trajectory: iter 027 (#7, −1) → 34; iter 028 (#19+#20, −2) → 32; iter 029 (DV2-R01, −1) → 31; iter 030 (#51, −1) → 30; iter 031 (DV2-R02+R03, −2) → 28. Hitting prior ≤15-by-iter-035 target requires ~13 more closures in iter 032-035 (achievable only with additional multi-item bundled iterations; may need to relax target to ≤15-by-iter-038). Reverse portfolio-drift trigger (D-1) remains armed at iter 026 close (process-engine is extension-adjacent, not D-1-enumerated tracked extension surface); clears at iter 027 close with #7 policy-engine touch as programmed. **DASHBOARD_V2_REVIEW_001 cold pool** holds 24 additional items (11 P1, 10 P2, 3 P3) per MR-005 D-5 Audit-Intake Pattern; promote via (path 1) P0 burn-down creating slot OR (path 2) PRD-trigger enumerated dependency.
...
- Follow-up pool at **35 items** post-DASHBOARD_V2_REVIEW_001 intake (32 at iter 026 close + 3 P0 promotions DV2-R01/R02/R03). Pool-size ceiling rule still violated (pool > 8 soft; > 15 hard). MR-005 Change D-2 hard-stop ceiling (pool > 15) applies inside Mode 5 only; no Mode 5 expected through iter 028. Post-intake burn-down programming projects pool ≤31 by iter 029 close, ≤28 by iter 031 close. Prior ≤15-by-iter-035 target now optimistic; realistic target ≤15-by-iter-038.
...
- Reverse portfolio-drift trigger (MR-005 Change D-1, N=5) **remains armed after iter 026 close** — iter 026 touched process-engine which is extension-adjacent but NOT enumerated in D-1's tracked extension surface list (`extension-app / segmentation-engine / normalization-engine / policy-engine`). Partial relief only. Full relief arrives at iter 027 with #7 policy-engine touch as programmed. **Iter 027 Candidate Selection does NOT require `reverse-portfolio-drift: user-ack`** because the trigger clears on selection of a D-1-enumerated surface.
...
- Agent-diversity: `backend-engineer` consecutive counter = 1 post-iter-026. If iter 027 + iter 028 both primary `backend-engineer`, same-implementer-4+ trigger fires at iter 029 (coordinator MUST rotate iter 029 regardless of other selection rules). Note: DV2-R01 (iter 029 recommended) is analytics-ops / server-side script — appropriate for `backend-engineer` OR `analytics` agent; rotation to `analytics` preserves both rules.
